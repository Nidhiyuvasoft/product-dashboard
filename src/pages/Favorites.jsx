          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6">
              {favorites.map((product) => (
                <div
                  key={product.id}
                  className="border rounded-lg shadow-sm bg-white hover:shadow-md transition-shadow p-3 sm:p-4 flex flex-col items-center h-full"
                >
                  <img
                    src={product.image}
                    alt={product.title}
                    className="h-32 sm:h-40 w-full object-contain mb-3 sm:mb-4"
                  />
                  <h2 className="font-bold text-lg text-center line-clamp-2 flex-1">
                    {product.title}
                  </h2>
                  <p className="mt-1 sm:mt-2 font-semibold text-m sm:text-base">
                    ${product.price}
                  </p>
                  <button
